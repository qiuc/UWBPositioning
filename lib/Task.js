...
Task.prototype._handleProtocolPacket = function(packet) {
    ...
    switch(packet.payload.name) {
        case "Tof Report":
            /*
            if(this._anchorCluster.isPositioned({address: '127.0.0.1', port: 1234}, packet.header.anchorId)) {
                this._tagCluster.processTofReport(packet.header.anchorId, packet.payload);
            }
            */
            task.Anchor.isPositioned(packet.header.anchorId, function(err, result) {
                if(err) return task.emit('error', err);
                if(result) {
                    task.Tag.setTofData(packet.header.anchorId, packet.payload, function(err) {
                        if(err) task.emit('error', err);
                    });
                }
            });
